...
public class DigitalBloomEngine {
    private var rustEngine: UnsafeMutablePointer<OpaqueDigitalBloom>?

    /// Animation modes available
    public enum AnimationMode: UInt8, CaseIterable {
        case vine = 0
        case gravity = 1
        case bounce = 2
        case burst = 3
    ...
    public func getParticleCount() -> Int {
        ...
        return Int(digital_bloom_get_particle_count(engine))
    }
    ...
    public func getParticles(maxParticles: Int = 500) -> [Particle] {
        ...
        let count = buffer.withUnsafeMutableBufferPointer { bufferPtr -> Int in
            return Int(digital_bloom_get_particles(engine, bufferPtr.baseAddress, UInt(maxParticles)))
        }
